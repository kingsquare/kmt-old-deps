...
/***
 * Hacks by Kingsquare to make it more apc.php-like
 * * added defaults() function and replaced the defines with defaults...
 * * added check for USE_AUTHENTICATION like apc does
 * * added slab link to scroll to opened slab directly
 */

function defaults($d,$v) {
	if (!defined($d)) define($d,$v); // or just @define(...)
}
defaults('USE_AUTHENTICATION',1); 	// check for username/password
defaults('ADMIN_USERNAME','nonrandomstringjusttobesure'); 	// Admin Username
defaults('ADMIN_PASSWORD','withanothernonrandomstring');  	// Admin Password
defaults('DATE_FORMAT','Y/m/d H:i:s');
defaults('GRAPH_SIZE',200);
defaults('MAX_ITEM_DUMP',50);
...
///////////////// Password protect ////////////////////////////////////////////////////////////////
if (USE_AUTHENTICATION && (!isset($_SERVER['PHP_AUTH_USER']) || !isset($_SERVER['PHP_AUTH_PW']) ||
...
<div class="head">
    ...
    <div class="kmt">
		<a href="/kmt/kingsquare/stats/">Terug naar KMT</a>
	</div>
    <hr class="memcache">
</div>
<div class=content>
...
if (empty($MEMCACHE_SERVERS)) {
	echo 'No cache servers configured';
	exit;
}
switch ($_GET['op']) {
...
EOB;
        foreach($MEMCACHE_SERVERS as $i => $server){
            echo '<table cellspacing=0><tbody>';
            echo '<tr class=tr-1><td class=td-1>'.$server.'</td><td><a href="'.$PHP_SELF.'&server='.$i.'&op=6">[<b>Flush this server</b>]</a></td></tr>';
...
EOB;
			...
			$serverKey = array_search($server, $MEMCACHE_SERVERS);
            foreach($entries as $slabId => $slab) {
                $dumpUrl = $PHP_SELF.'&op=2&server='.$serverKey.'&dumpslab='.$slabId.'#'.$serverKey.'_'.$slabId;
                echo
                    "<tr class=tr-$m>",
                    "<td class=td-0><center>",'<a name="',$serverKey,'_',$slabId,'" href="',$dumpUrl,'">',$slabId,'</a>',"</center></td>",
                    ...
                    if ((isset($_GET['dumpslab']) && $_GET['dumpslab']==$slabId) &&  (isset($_GET['server']) && $_GET['server']==$serverKey)){
                        ...
                        foreach($items['ITEM'] as $itemKey=>$itemInfo){
                            ...
                            echo '<a href="',$PHP_SELF,'&op=4&server=',$serverKey,'&key=',base64_encode($itemKey).'">',$itemKey,'</a>';
                            if ($i++ % 10 == 0) {
                                echo '<br/>';
                            }
