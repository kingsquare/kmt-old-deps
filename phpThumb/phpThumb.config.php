...
/**
 * This file is no longer loaded for KMT since we use the object-wise method!
 */
...
$PHPTHUMB_CONFIG['document_root'] = FS_FILE_CACHE; //realpath((getenv('DOCUMENT_ROOT') && ereg('^'.preg_quote(realpath(getenv('DOCUMENT_ROOT'))), realpath(__FILE__))) ? getenv('DOCUMENT_ROOT') : str_replace(dirname(@$_SERVER['PHP_SELF']), '', str_replace(DIRECTORY_SEPARATOR, '/', dirname(__FILE__))));
...
$PHPTHUMB_CONFIG['cache_directory'] = sys_get_temp_dir().'/phpThumb/';                            // set the cache directory relative to the phpThumb() installation
...
$PHPTHUMB_CONFIG['cache_maxage'] = null;        // Kingsquare: intentional null: borkt de pan uit

//$PHPTHUMB_CONFIG['cache_maxsize'] = null;           // never delete cached thumbnails based on byte size of cache directory
$PHPTHUMB_CONFIG['cache_maxsize'] = null; // delete least-recently-accessed cached thumbnails when more than [1000MB] of cached files are present (value is maximum bytesize of all cached files)

//$PHPTHUMB_CONFIG['cache_maxfiles'] = null;          // never delete cached thumbnails based on number of cached files
$PHPTHUMB_CONFIG['cache_maxfiles'] = null;             // delete least-recently-accessed cached thumbnails when more than [20000] cached files are present (value is maximum number of cached files to keep)
...
$PHPTHUMB_CONFIG['cache_prefix'] = 'phpThumb_';
