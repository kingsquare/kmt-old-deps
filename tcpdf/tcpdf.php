...
class TCPDF {
		...
		if ($file{0} === '@') {
			...
			if ($imsize === FALSE) {
				if (function_exists('curl_init')) {
					...
					//curl_setopt($cs, CURLOPT_VERBOSE, true);
					//curl_setopt($cs, CURLOPT_STDERR, fopen('/tmp/kmt-curl.log', 'w+'));
	 ...
	 * @param $dest (string) Destination where to send the document. It can take one of the following values:<ul>
									...
									switch ($strpiece[2][0]) {
										...
										case 're': {
											...
											$pmid = preg_replace_callback('/('.$xmatches[1].')[\s]('.$xmatches[2].')[\s]('.$xmatches[3].')[\s]('.$strpiece[1][0].')[\s](re)([\s]*)/x',
												static function ($matches){
                                                    global $x_diff, $w_diff;
                                                    $newx = sprintf('%.2F', (float)$matches[1] + $x_diff);
                                                    $neww = sprintf('%.2F', (float)$matches[3] + $w_diff);
