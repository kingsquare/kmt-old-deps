...
class Cpdf {
	...
	function o_info($id,$action,$options=''){
	  ...
	  switch ($action){
	    case 'new':
	      $this->infoObject=$id;
	      $date='D:'.date('Ymd');
	      $this->objects[$id]=array('t'=>'info','info'=>array('Creator'=>'Kingsquare Information Services http://www.kingsquare.nl/','CreationDate'=>$date));
	      break;
	    case 'Title':
	    case 'Author':
	    case 'Subject':
	...
	*/
	function addJpegFromFile($img, $x = false, $y = false, $w=0, $h=0){
	  ...
	  if ($x===false) $x = $this->x - $w;
	  if ($y===false) $y = $this->y - $h;
