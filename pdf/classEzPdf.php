<?php
include_once __DIR__.'/classPdf.php';
...
// mod by Reggino, eenvoudige hack, werkt alleen voor volgbladen!
var $letterPaper=false;
...
//echo 'left='.$this->ez['leftMargin'].'   right='.$this->ez['rightMargin'].'<br>';
  ...
  if ($pageRequired){
    ...
    if ($this->letterPaper) {
		$fileTool = Toolbox_kmt::__getTool('file');
		if ($fileTool->getMimeType($this->letterPaper) == 'image/png') {
			$this->addPngFromFile($this->letterPaper,0,0,595.4,842);
		} else {
			$this->addJpegFromFile($this->letterPaper,0,0,595.4,842);
		}
    }
  } else {
    $this->y = $this->ez['pageHeight']-$this->ez['topMargin'];
